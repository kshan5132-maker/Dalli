...
## [Beta 1.0.10 - 피드백 반영] - 2026-03-21

### 신규/개선 기능 (5건)

#### 1. PWA 홈 바로가기
- **변경**: manifest.json 강화 + 자동 생성 아이콘 (icon.tsx, apple-icon.tsx)
- **효과**: 카카오톡 링크 → 브라우저 → "홈 화면에 추가"로 앱 아이콘 생성 가능
- **파일**: `public/manifest.json`, `src/app/icon.tsx`, `src/app/apple-icon.tsx`

#### 2. 좋아요/싫어요 디자인 유튜브 스타일로 변경
- **변경**: 이모지(👍👎) → SVG 아이콘 + 라운드 필(pill) 디자인
- **활성화**: 누를 때 아이콘 fill + 색상 변경 (좋아요=파랑, 싫어요=빨강)
- **구분선**: 좋아요/싫어요 사이 세로 구분선(|) 추가
- **파일**: `src/app/group/[id]/page.tsx`

#### 3. 좋아요/싫어요 누가 눌렀는지 확인
- **변경**: 좋아요/싫어요 숫자 클릭 시 리액션 상세 모달 표시
- **내용**: 좋아요 목록 (닉네임 + 프로필 이니셜), 싫어요 목록 분리 표시
- **파일**: `src/app/group/[id]/page.tsx`

#### 4. 피드 공유하기 기능
- **변경**: 피드 카드에 "공유" 버튼 추가
- **동작**: Web Share API 지원 시 네이티브 공유, 미지원 시 클립보드 복사
- **공유 내용**: "[닉네임]님이 [루틴](운동종류 운동량)을 인증했어요! - Dalli"
- **파일**: `src/app/group/[id]/page.tsx`

#### 5. 하루 다회 인증(피드) 허용
- **변경**: 하루 1회 인증 제한 해제 → 여러 번 피드 올리기 가능
- **인증 횟수**: 주간 달성률 카운트는 여전히 하루 1회만 인정 (기존 로직 유지)
- **안내 문구**: "오늘 인증 완료! 추가 인증은 피드에만 표시돼요"
- **버튼**: "오늘 인증 완료됨"(비활성) → "추가 인증하기"(활성)로 변경
- **파일**: `src/app/verify/page.tsx`

---

## [Beta 1.0.9 - 마일리지 탭] - 2026-03-18

### 신규 기능

#### 마일리지 대시보드 (캘린더 + 주간/월간 통계)
- **위치**: 대시보드 페이지에 3번째 탭 "마일리지" 추가 (개인 | 그룹 | 마일리지)
- **캘린더**: 월 네비게이션(←/→)으로 과거/미래 월 이동, 인증 있는 날 초록 점 표시
- **날짜 선택 연동**: 선택한 날짜 기준으로 주간/월간 마일리지 자동 갱신
  - 주간: 선택 날짜가 속한 월~일 범위의 인증 집계
  - 월간: 캘린더에 표시된 달 전체 인증 집계
- **운동 종류별 breakdown**: 웨이트, 런닝, 수영, 자전거, 등산, 요가, 기타별 횟수 + 운동량 표시
- **일별 상세**: 선택 날짜의 인증 목록 (운동 아이콘, 종류, 운동량, 루틴명, 메모, 시간)
- **개인+그룹 통합**: 사용자의 모든 인증(개인/그룹 무관) 데이터 통합 조회
- **인터랙션**: 월 이동 시 해당 월 1일 자동 선택 (현재 달이면 오늘 선택), 선택 주 하이라이트
